...
| [square-ladders](square-ladders/) | [A089761](https://oeis.org/A089761): the least k such that k·i²+1 is prime for every i = 1..n — a Dickson ladder whose rungs are the squares | **PAUSED — open to others** — **a(16) = 15,737,271,507,027,492 and a(17) = 125,811,821,444,034,258** found & verified 2026-08-21/22 in a 13.5-hour campaign — the first terms of this sequence found by anyone since 2008, and the first break in a five-term plateau. All values are **proved** prime, and since the v4 engine that is true by construction: the enforced ceiling *is* the deterministic Miller-Rabin bound rearranged, k_ceil(n) = (3.317×10²⁴−2)/n²+1. Prior frontier: a(11) (Donovan Johnson, 2008) was the last term anyone searched for, with a(12)–a(15) the *same integer* riding on it; a(16) was open because of one composite, 861066640·16²+1 = 47·149·31476947, and Alekseyev's a(16) > 1.4×10¹³ was the only work since. Model quantiles 0.899 and 0.530 — too few to score. The campaign paused at k = 9.65×10¹⁸, not on a find. **v4** carries (k, off) and reduces the offset instead of the absolute k (OPTIMIZATION.md 2.7), lifting the ceiling from a machine word to 1.02×10²² — a 1,138× larger range for a measured 1.8% of throughput. **v5** then took the wheel to 47 on three CRT levels: tests per unit of k line is exactly Σ S(q) over the primes above the wheel, and at n = 18 the three primes 41, 43, 47 are **72% of every test the engine did** — reachable only once the second-level table is factored again, into 4,560 × 16,675 instead of 76,038,000. **4.306× end to end**, interleaved and paired; SCORE3L 1,212,461,925, SCORE 261,712,890, 27 gates green. **a(18)** is now ~1.6 h to its median from the current cursor and 6 h to P90 |
