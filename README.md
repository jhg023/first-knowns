...
| [square-ladders](square-ladders/) | [A089761](https://oeis.org/A089761): the least k such that k·i²+1 is prime for every i = 1..n — a Dickson ladder whose rungs are the squares | **PAUSED — open to others** — **a(16) = 15,737,271,507,027,492, a(17) = 125,811,821,444,034,258 and a(18) = 109,927,810,420,106,024,208** found & verified 2026-08-21/23 in a 34.2-hour campaign across three engine versions — the first terms of this sequence found by anyone since 2008, and the first break in a five-term plateau. All values are **proved** prime, and since the v4 engine that is true by construction: the enforced ceiling *is* the deterministic Miller-Rabin bound rearranged, k_ceil(n) = (3.317×10²⁴−2)/n²+1. Prior frontier: a(11) (Donovan Johnson, 2008) was the last term anyone searched for, with a(12)–a(15) the *same integer* riding on it; a(16) was open because of one composite, 861066640·16²+1 = 47·149·31476947, and Alekseyev's a(16) > 1.4×10¹³ was the only work since. **a(18) is 5.96 × 2⁶⁴** — no engine before **v4** could address it, and v4 is what carries (k, off) and reduces the offset instead of the absolute k (OPTIMIZATION.md 2.7), lifting the ceiling from a machine word to 1.02×10²² for a measured 1.8% of throughput. **v5** then took the wheel to 47 on three CRT levels: tests per unit of k line is exactly Σ S(q) over the primes above the wheel, and at n = 18 the three primes 41, 43, 47 are **72% of every test the engine did** — reachable only once the second-level table is factored again, into 4,560 × 16,675 instead of 76,038,000. **4.306× end to end**, interleaved and paired; the a(18) sweep ran 1.00×10²⁰ of line in 20.7 h at **1.35×10¹⁵ k/s**. Model quantiles 0.899, 0.530 and **0.9996**: three finds cost 10.98 expected hits, so this model runs **3.7× hot** and its 95% interval [1.25, 17.7] excludes 1 — while the campaign's own census of 86,531 classified survivors matches the same model at **1.022×**, so the intensity is right and it is the waiting time to a FIRST occurrence that runs long. SCORE3L 1,201,861,043, SCORE 266,058,288, 29 gates green. Paused at k = 1.10×10²⁰ with **a(19)** open: model median 2.72×10²⁰, ~33 h on one 4090, and the engine's n = 19 ceiling is 84× further out |
