...
| [primorial-ap](primorial-ap/) | [A053647](https://oeis.org/A053647): the first term of the first arithmetic progression of n primes whose common difference is the n-th primorial — the smallest difference such a progression can have | **ACTIVE** — no results yet: the engine is built, the gate battery is green and the odds model is validated, but **no production sweep has been run** (bounded smoke runs reach 6.4×10¹², verifying a depth-15 NEAR on the way). Frontier: Donovan Johnson's a(15) = 158,317,270,283, Oct 2009, a 17-year-old frontier with no published bound on any open term. The engine is a gated **v2** — pattern tables + shared-memory sub-segments, SCORE 5,163 → 39,772 with the stream pinned bit-for-bit — at **2.07×10¹¹ p/s end-to-end**, 34.7× the v1 it was first priced around. First target **a(16)**, model median 3.87×10¹³ — **three minutes** of sweep; a(17) is an hour, a(18) a day, a(19) three weeks at the median, so this is built to find **four** terms, with a(20) (median 1.5 y) the one that depends on work not yet done |
