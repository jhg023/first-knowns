...
**The noise floor is 12%, and it was measured rather than assumed.** In the
...
product under 512), and they measured 1.000 against 1.119. Everything in
the tables above under about 1.10 is therefore reported with that in mind,
and the decisions that turn on less than that were taken on the geometric
mean across four shapes and nine rounds, not on one number.
...
| extract & queue | ~12% | **UNSEARCHED, and named.** One warp-aggregated shared atomic per WPT words, behind a five-step shuffle prefix scan. The scan is per THREAD and so is charged per word rather than per candidate, which at a deep wheel (0.28 set bits per 32-bit word) is where its cost comes from. Two untried candidates: `__reduce_add_sync` for the warp total, and giving each warp an analytically-sized queue region so the prefix disappears. Neither has been measured |
