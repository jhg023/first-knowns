...
from huntlib.primes import (mr_is_prime, sprp_base2, factor_witness,
                            MR_VALID_BELOW)
from huntlib.certificate import prove, verify, factor_partial, factor_full
...
| `drills` | the repo-wide selftest drills every project owes (CONVENTIONS.md rule 6): the graceful shutdown, the 785-NUL checkpoint and its `.bak`, the checkpoint held open by another process (a **real** handle, not a synthetic errno — the failure that killed two live campaigns), the pool ramp, and the evidence upsert — plus `event_kind_drill`, which makes a project exercise all four outcomes of the discovery-once taxonomy rather than only the happy one. `standard()` returns them all as `(ok, msg)` pairs. They mutate process state deliberately and put it back, so a selftest can run them mid-battery |
