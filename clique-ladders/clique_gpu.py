"""clique_gpu.py -- the GPU engine for the clique ladders.
...
so the same code sieves every family and both extra forms.

THE ENGINE IS INHERITED: it is factorial-ladders' v3 (lcm-ladders' v1
window sieve, itself linear-ladders' v4, plus the 2^64 reduction bound and
the wide survivor record), with two changes -- the form list is a*x + b
built from the sequence's own terms rather than k!*x + s, and the unit is a
CLASS (below).  The kernel needed no change.  EVERY TUNING CONSTANT BELOW,
AND EVERY COMMENT THAT PRICES ONE, IS THOSE PROJECTS': the measurements they
quote ("at n = 17", "A078502", "the wheel to 53 at n = 18") were made on
lcm-ladders' and factorial-ladders' filters and are kept as the provenance
of the numbers, not as claims about this project.  They were RE-SWEPT HERE
in round 1 (OPTIMIZATION_LOG.md), on this project's openings and on stand-in
filters past them where the campaign's hours are: the block shape moved
(BLOCK_SHAPES_BY_NW), the sieve depth moved (SURV_TARGET_DEEP), the planner
was replaced (`plan`), and every other constant read unchanged.

CLASS SPACE.  The forced primes pin x to ONE residue class, x == r (mod u),
and r is generally not 0 (clique_search.forced_class: 2 mod 6 for A093483,
9 mod 30 for A103828), so where the inherited engine swept the multiples of
a unit this one sweeps a class: the device runs t with x = r0 + unit*t, the
kill sets carried through the map (K'(q) = (K(q,n,F) - r0) * unit^-1 mod q)
and the unit's primes left out of the wheel.  Everything the device touches
is in t; `self.W` (the period), the survivors and every bound check are in
x, and `_collect` is the one place the map is applied.  Because r0 < unit,
x < j*W exactly when t < j*W', so the periods of x and of t share their
boundaries and no cursor changes meaning.  `unit = 1` is the x-space engine,
and the gate battery runs it on wheels a dense CPU sieve can follow.
`assert_unit` refuses a unit with a prime that is not forced at the filter,
or not squarefree, which is the one way this could thin the line.
...
a(1..n-1) (clique_reference.forms), so an engine exists only for an index
...
CEILINGS, stated and enforced (CONVENTIONS.md "Numeric hygiene"):
  * x < k_ceil(n, F) = huntlib.ceiling.K_CEIL = 1e40 for every family
    (clique_search, G10).  The values here are UNSTRUCTURED, so this is
    rule 5h's exception and rests on a measurement G10 repeats.
...
from clique_reference import (FAMILIES, FOUND, KNOWN, family,  # noqa: E402
                              forbidden_k_residues, frontier, nforms,
                              term, w_count, wheel_residues)
from clique_search import (Q2_DEFAULT, CpuEngine, assert_unit,  # noqa: E402
                           forced_unit, k_ceil, k_floor, killed_residues,
                           unit_residue)

# The two families the mechanism gates run on: no extra form, forced unit
# 6, and open indices (19 and 18) high enough for every filter the gates
# name.  The families WITH an extra form (2x + 1; x itself) are in G9.
FA, FB = "A037100", "A093483"
...
#
# The forced class is 6 or 30 at the open indices and may grow with the
# sequence, and the kill sets grow with every term: K(q,n) is a subset of
...
def plan_q2(n, fam, unit, wheel, target=None, ladder=Q2_LADDER):
    ...
    # does not divide the unit (clique_reference G2b proves the count
...
#
#     clique_model.expected_sweep(segment)  x  density(wheel) / crel
#
...
# candidate rate, is MEASURED, paired and interleaved on this project's
...
# (same log).
CREL_WIDTH = {32: 0.665, 64: 0.858, 96: 0.952, 128: 1.000, 160: 0.985,
              192: 1.02, 224: 1.06}
CREL_WIDE = 0.93
# a narrow window whose last word is part live (179 periods in 192 on the
# wheel to 47) measured 0.981 of the 128-period rate
CREL_NARROW_CLAMPED = 0.981
...
@_lru_cache(maxsize=None)
def plan(n, fam, unit, r1_max=R1_MAX, top=WHEEL_TOP):
    ...
    for lv, Wp, dens in wheel_candidates(n, fam, unit, r1_max, top):
        ...
        for pb in range(PV_MIN, PB_DEFAULT + 1, 32):
            ...
            crel = CREL_NARROW_CLAMPED if pv < pb else CREL_WIDTH[pb]
            if wide:
                crel *= CREL_WIDE
            crel /= max(lit, 1)
...
# NW >= 5 only, and this project's planner lands on every width.  Swept
...
extern "C" __global__ void sieve(
        ...
        const unsigned int* __restrict__ patr%(gparams)s)
{
%(qdecl)s
    __shared__ int q3b;
...
#pragma unroll
            for (int i = 0; i < NW; ++i) {
                ...
                while (a) {
                    ...
                    if (p < Q0CAP) { qi0[p] = QIDX(ss) | BWBIT(e1.y < dd);
...
class GpuEngine:
    ...
    def __init__(self, n, fam, p1=None, p2=None, p3=None,
                 ...
                 wide=None, seg_cap=None, xe=None):
        import cupy as cp
        self.cp = cp
        self.n = int(n)
        self.fam = family(fam)
        ...
        # (clique_search.killed_residues).  r0 < unit, so the periods of x
        ...
        def _nonq():
            return (self.xe * self.nw * tpb * 4              # sal
                    + self.spb * ng4_ * 4                    # ne
                    + self.spb * 4                           # d2
                    + (self.spb * (len(self.groups2) + 40) * 2   # ne2
                       if ROUND_WINDOW and self.wide else 0)
        ...
        self.rwin = bool(ROUND_WINDOW and self.wide and self.groups2
        ...
        if self.rwin:
            for gi, (Q, dinv) in enumerate(zip(self._g2q_list(), self._dinvs2)):
                ...
                red2 = (f"r = (unsigned int)xx - "
                        ...
                        f"if (r >= {Q}u) r -= {Q}u; ")
                blockpre.append(
                    f"        {{ unsigned long long xx = (unsigned long long)"
                    f"{self.W1}u * (unsigned long long)dcur; {red}"
                    f"xx = (unsigned long long)r * {dinv}ULL; {red2}"
                    f"const unsigned int bs = (jmod2[{gi}] + shift) % {Q}u; "
                    f"nv[{self.slot2[gi]}] = (bs + {Q}u - r) % {Q}u; }}")
    ...
    def _enqueue(self, b, gx, gy, gz, nper, u0, t_lo, gps):
        ...
        self.k_sieve((gy, gx, gz), (self.tpb,),
                     (np.int32(self.R1), np.int32(self.R2), np_,
                      ...
                      self.d_x0r, self.d_jmod2, self.d_patr, *gps))
        R = len(self.rounds)
...
def g7_wheel_matches_oracle():
    ...
    wheel exactly."""
    fams = (FA, FB, "A119752", "A133761")
    for fam in fams:
        top = frontier(fam) + 1
        for n, p1 in ((5, 7), (9, 11), (15, 11), (15, 13), (top, 13)):
        ...
        for n, p1, unit in ((11, 13, 6), (15, 13, 6), (top - 1, 13, 6),
        ...
        for n, p1, p2, unit in ((15, 11, 13, 6), (top - 1, 13, 19, 6),
                                (top, 11, 13, 6)):
    ...
    for n, unit in ((11, 42), (17, 14), (15, 12), (1, 6)):
        try:
    ...
    return True, ("G7 ok: CRT-lifted wheel == oracle period walk at (n,p1) = "
                  "(5,7), (9,11), (15,11), (15,13) and each open index, for "
                  "four families (both extra forms included); the two-level "
                  ...
                  "and recombines across two levels; and the units 42, 14, 12 "
                  "and 6-at-n=1 are REFUSED, since a prime of each is not "
...
def g8_wheel_partitions_the_period():
    ...
    """
    from clique_reference import w as w_formula
    cases = []
    for fam in FAMILIES:
        top = frontier(fam) + 1
        cases += [(fam, 15, 13, 1, 1), (fam, 15, 23, 1, 1), (fam, 12, 23, 1, 1),
                  (fam, top, 19, 1, 1), (fam, 15, 37, 23, 1),
                  (fam, 15, 31, 23, 1), (fam, top, 47, 37, 1)]
...
def g9_gpu_matches_cpu():
    ...
    """
    from clique_search import k_proof
    cross_17 = k_proof(17, FA)          # 9.3e9, a crossing the hunt passes
    ceil = k_ceil(15, FA)               # 1e40, the enforced ceiling
    W19 = 9_699_690
    cases = (
        ...
        (FB, 9, 13, None, None, 32, 2 * 10 ** 9, 2 * 10 ** 7, 1),
        (FA, 15, 13, None, None, 32, 10 ** 12, 10 ** 7, 1),
        (FB, 15, 13, 23, None, 32, 10 ** 12, 10 ** 7, 1),
        (FA, 15, 11, 19, 23, 32, 10 ** 12, 10 ** 7, 1),
        (FB, 17, 13, 23, None, 32, 9 * 10 ** 14, 2 * 10 ** 7, 1),
        (FA, 16, 13, 19, 23, 32, 9 * 10 ** 14, 2 * 10 ** 7, 1),
        ...
        (FB, 15, 13, 19, None, 32, 64, W19 - 64, 1),
        ...
        (FB, 15, 13, 19, 23, 32, 10 ** 12, 10 ** 7, 6),
        (FA, 16, 13, 19, 23, 32, 10 ** 12, 2 * 10 ** 7, 6),
        (FB, 16, 13, 23, None, 32, 10 ** 12, 2 * 10 ** 7, 6),
        (FA, 17, 13, 19, 23, 32, 10 ** 15, 2 * 10 ** 7, 6),
        (FB, 18, 13, 23, None, 32, 10 ** 15, 2 * 10 ** 7, 6),
        (FA, 19, 11, 17, 23, 32, 10 ** 15, 4 * 10 ** 7, 6),
        ...
        # there.
        (FB, 17, 13, 19, 23, 32, cross_17 + 10 ** 12, 2 * 10 ** 7, 6),
        (FA, 15, 13, 19, None, 32, 10 ** 21, 10 ** 7, 6),
        ...
        (FA, 15, [5, 7, 13, 17, 19], [23, 29], None, 32,
         10 ** 12, 10 ** 7, 6),
        (FB, 17, [5, 7, 13, 17], [19, 23], [29], 32,
         10 ** 15, 2 * 10 ** 7, 6),
        # THE FAMILIES WITH AN EXTRA FORM -- 2x + 1 (a leading coefficient
        # that is not 1, invisible to q = 2) and x itself (offset 0) -- and
        # the forced classes mod 30, in class space and in x space
        ("A119752", 15, 13, 19, None, 32, 10 ** 12, 10 ** 7, 6),
        ("A119751", 15, 13, 19, 23, 32, 10 ** 15, 2 * 10 ** 7, 30),
        ("A133761", 17, 13, 19, 23, 32, 10 ** 15, 2 * 10 ** 7, 30),
        ("A103828", 19, 13, 23, None, 32, 10 ** 18, 2 * 10 ** 7, 30),
        ("A119751", 15, 13, 23, None, 32, 10 ** 12, 10 ** 7, 1),
        ("A133761", 17, 13, 23, None, 32, 10 ** 12, 10 ** 7, 1),
        # HARD AGAINST THE 1e40 CEILING, on both signs: the base is a
        # 133-bit Python int on both engines
        (FA, 15, 13, 19, 23, 32, ceil - 10 ** 9, 10 ** 7, 6),
        (FB, 16, 13, 19, 23, 32, ceil - 10 ** 9, 2 * 10 ** 7, 6),
    )
    total = 0
    ...
    return True, (f"G9 ok: GPU stream == CPU stream on {len(cases)} populated "
                  ...
                  f"wheels in x space AND in class space (mod 6 and mod 30), "
                  f"SUBSET wheels that decline 11 to the sieve, all six "
                  f"families and both extra forms (2x + 1; x itself), filters "
                  f"n = 9 to 19, heights 2e9 -> {ceil:.3g} with windows ABOVE "
                  f"2^64 and past the proof crossing ({cross_17:.3g}), two "
                  f"hard against the 1e40 ceiling; and period 0 clipped at "
                  f"the engine floor")
...
def g13_production_wheel_constants():
    ...
    from clique_reference import forced_primes, w as w_formula
    cases = [(FA, 15, 23, 37, 47, 1), (FA, 16, 23, 37, 47, 1),
             (FB, 15, 23, 37, 47, 1), (FB, 16, 23, 37, None, 1),
             ("A119751", 15, 23, 37, 47, 1), ("A133761", 17, 23, 37, 47, 1)]
    ...
    return True, (f"G13 ok: the wheel constants (W1, W2a, W2b, W1^-1 and the "
                  ...
                  f"x-space (23,37,47] at n = 15..17 of four families, and "
...
def g14_engine_mechanisms():
    ...
    cases = ((FA, 15, 23, 37, None, 65536, 1),
             (FA, 16, 23, 37, 47, 65536, 1),
             (FB, 15, 23, 37, 47, 65536, 1),
             (FA, 15, 23, 31, None, 4096, 1),
             # the unit wheels the campaigns run
             (FA, 15, 19, 31, 43, 131072, 6),
             (FA, 17, 19, 31, 43, 32768, 6),
             (FB, 15, 19, 31, 43, 131072, 6),
             (FB, 16, 23, 37, 47, 131072, 6),
             (FA, 18, 23, 37, 47, 32768, 6),
             (FB, 17, 19, 29, 41, 65536, 6))
    ...
    ref = GpuEngine(15, FA, p1=13, p2=23, p3=None, q2=128)
    tiny = GpuEngine(15, FA, p1=13, p2=23, p3=None, q2=128,
                     qcap_sigma=-1e9)
    ...
    return True, (f"G14 ok: the split A/C/D generation tables reproduce the "
                  ...
                  f"space (mod 6) wheels at n = 15, 16, 17, 18; the compaction "
                  f"depths derive from the survival curve ({FA} n = 17: "
...
def g15_k_off_representation():
    ...
    eng = GpuEngine(15, FA, p1=13, p2=23, p3=None, q2=512)
    engb = GpuEngine(15, FA, p1=13, p2=23, p3=None, q2=512, pb=32,
                     tchunk=TPB_DEFAULT, nu=1, wide=True)
    engn = GpuEngine(15, FA, p1=13, p2=23, p3=None, q2=512, pb=32,
    ...
    engu = GpuEngine(15, FA, p1=19, p2=23, p3=None, q2=512, unit=6)
    engub = GpuEngine(15, FA, p1=19, p2=23, p3=None, q2=512, unit=6,
    ...
    if oneu != manyu or oneu != sorted(partsu):
        return False, (f"G15 FAIL: with unit 6 the stream depends on the "
                       ...
                       f"{len(partsu)} split")
    if any(k % 6 != engu.r0 for k in oneu):
    ...
    return True, (f"G15 ok: the survivor stream is INVARIANT under the launch "
                  ...
                  f"the forced class mod 6, with the residue lists the class-space "
...
def g16_third_level_mechanisms():
    ...
    """
    n, fam, p1, p2, p3, q2 = 15, FA, 13, 17, 19, 128
...
def g17_unit_wheel_matches_k_wheel():
    ...
    for n, fam in ((15, FA), (17, FB)):
        ...
        if any(k % unit != prod.r0 for k in a):
    ...
    return True, ("G17 ok: four wheels, four arithmetics, one stream -- the "
                  "class-space wheel to 43, the same prime set split at a "
                  ...
                  + "; ".join(out) + " -- every survivor in the forced class, and "
...
def g18_every_opening_compiles_to_occupancy():
    ...
    return True, (f"G18 ok: all {len(rows)} campaign configurations (each "
                  f"family's open index -- the one filter a campaign can open "
                  f"at -- and the two before it, each PLANNED "
...
def g19_reduction_bound_is_the_word():
    ...
    span = 256
    for pb in (64, 256):
        eng = GpuEngine(17, FA, p1=W47[0], p2=W47[1], p3=W47[2],
    ...
    try:
        ...
        try:
            GpuEngine(17, FA, p1=W47[0], p2=W47[1], p3=W47[2],
...
def g20_wide_wheel_matches_narrow_wheel():
    ...
    # q2 = 64 with the window cut short by hand, so six primes go through
    ...
    span2 = 3 * 10 ** 8
    cpu2 = CpuEngine(n, fam, q2=64)
    want2 = [k for c in cpu2.survivors(x_lo, x_lo + span2) for k in c]
    if not want2:
        return False, "G20 FAIL: the round drill's CPU window is empty"
    for wide, sig in ((False, QCAP_SIGMA), (True, QCAP_SIGMA), (True, -1e9)):
        eng = GpuEngine(n, fam, p1=NC[0], p2=NC[1], p3=NC[2], q2=64,
                        unit=unit, wide=wide, lit=2, qcap_sigma=sig)
        if eng.rwin != (wide and ROUND_WINDOW) or len(eng.qcaps) < 4:
            return False, (f"G20 FAIL: the round drill's engine (wide = "
                           f"{wide}) came up with rwin = {eng.rwin} and "
                           f"{len(eng.qcaps)} queues -- it would not exercise "
                           f"the window rounds and the aliased buffers")
        ...
        if got2 != want2:
            return False, (f"G20 FAIL: the in-block rounds on the "
                           f"{'wide' if wide else 'narrow'} record (rwin = "
                           f"{eng.rwin}, queues {eng.qcaps}) give "
                           f"{len(got2)} survivors, the CPU engine "
                           f"{len(want2)}: diff "
    ...
    return True, (f"G20 ok: the non-contiguous split {NC} at unit 6, n = {n}, "
                  ...
                  f"{len(want2)} at q2 = 64 as Barrett rounds (narrow), as "
                  f"window rounds (wide) and with the queues forced full; and at every open index the planned wheel "
