...
CEILINGS, stated and enforced (CONVENTIONS.md "Numeric hygiene"):
  ...
  * (seg_periods + 1) * W' + q2 < 2^64 = REDUCE_MAX on the DEVICE period
    W' (x' units): the largest offset the Barrett tail reduces is a
    segment plus one period wide, it is carried as a u64, and one
    conditional subtraction is exact for EVERY u64 (the bound is 2^64, not
    the 2^63 the engine shipped with: OPTIMIZATION_LOG.md round 2, with
    the paper bound, the bit-exact emulation and the tripwire in G19).
    That is what caps the window width on the full wheel to 47
    (W' = 1.0e17: 179 periods).
...
and the clamp exactly at the bound), G17
...
/* One test against prime IDX (unchanged from v3): the uint4 record
   ...
   rarely, its residue list.  ONE conditional subtraction is exact for
   every u64 (REDUCE_MAX = 2^64; the reduced quantity is the offset within
   the launch, bounded by the engine to keep (segments*PB + 1)*Wp + q2
   under it). */
#define TEST(IDX, DST) { \
...
class GpuEngine:
    ...
    def __init__(self, n, fam, p1=None, p2=None, p3=None,
        ...
        # off + j*W' for j below the segment's live periods, the offset is
        # a u64 and one conditional subtraction is exact for every u64, so
        # (PV + 1) W' + q2 < 2^64 bounds PV: 179 on the full unit wheel to
        # 47 (W' = 1.0e17), 28 on the k-space wheel to 47 (W' = 6.2e17) the
        # gates run.  Bits past PV in the window are masked off; the word
        # count follows PV, so a wheel that admits few periods gets a
        # 32-bit window rather than an idle 224-bit one.
        ...
        if pb is None:
            # A WINDOW CUT BY THE REDUCTION BOUND IS TAKEN IN FULL, with its
            # last word partial.  v1 rounded it down to a whole word on a
            # measured tie between 89 and 64 periods under the 2^63 bound;
            # under the 2^64 bound the full wheel to 47 admits 179, and 179
            # against 160 read 1.050 at n = 17 (five interleaved rounds),
            # 0.992 at n = 18 and 1.012 at n = 16 (OPTIMIZATION_LOG.md round
            # 2).  The segment is the coverage unit, so a find costs up to
            # one of them in over-sweep -- but since round 2 the launcher
            # carries the classified line across a promotion, so that
            # over-sweep is the next filter's work done early rather than
            # work thrown away.
...
def g19_reduction_bound_is_the_word():
    ...
    offsets pass 2^63.
      ...
      3. Device: the planned full wheel at n = 17, the first launch (a
         t-chunk of one block, u = 0) over the SAME 179 absolute periods
         swept at pb = 64 (three segments) and at pb = 192 (one segment of
         pv = 179): identical, non-empty, and the segment provably puts
         offsets above 2^63 (179 W' > 2^63).
      4. An engine asked for more than the bound admits clamps pv to
         exactly floor((2^64 - q2) / W') - 1: one more period would break
         the invariant the assert in __init__ states.
    """
    ...
    for pb in (64, 192):
        ...
        while j < 1 + span:
            ...
            j += seg
        sets[pb] = sorted(got)
    if pvs[192] != 179 or 179 * eng.Wp <= (1 << 63):
        return False, (f"G19 FAIL: the full wheel admits pv = {pvs[192]} at "
                       f"pb = 192 (expected 179), or 179 periods of W' = "
                       f"{eng.Wp} do not pass 2^63 -- the window does not "
                       f"exercise the lifted bound")
    if not sets[64]:
        return False, "G19 FAIL: the cross-width window is empty -- vacuous"
    if sets[64] != sets[192]:
        a, b = set(sets[64]), set(sets[192])
        return False, (f"G19 FAIL: pb = 64 ({len(a)} survivors) and pb = 192 "
                       f"({len(b)}) disagree over the same 179 periods: diff "
                       f"{sorted(a ^ b)[:4]}")
    # 4. the clamp is exactly the bound
    eng = GpuEngine(17, "A177013", p1=W47[0], p2=W47[1], p3=W47[2],
                    q2=1024, unit=6, pb=256, tchunk=TPB_DEFAULT, nu=1)
    want = (REDUCE_MAX - eng.q2) // eng.Wp - 1
    if (eng.pv != want or (eng.pv + 1) * eng.Wp + eng.q2 >= REDUCE_MAX
            or (eng.pv + 2) * eng.Wp + eng.q2 < REDUCE_MAX):
        return False, (f"G19 FAIL: asked for 256 periods the engine took "
                       f"pv = {eng.pv}, not the bound's own {want}")
    return True, (f"G19 ok: the u64 Barrett reduction with one conditional "
                  ...
                  f"wheel's first launch over 179 periods (offsets to "
                  f"{179 * eng.Wp:.3e} > 2^63) is identical at pb = 64 and "
                  f"pb = 192 ({len(sets[64])} survivors), and an engine asked "
                  f"for 256 periods clamps to exactly the bound's {want}")
