"""fladder_gpu.py -- the GPU engine for the factorial ladders.
...
candidates, and r'' is linear in the CRT decomposition of r, so per group
...
global TAIL ROUNDS over queues with several lanes per item.  Every push
into a queue is ONE shared atomic per warp (`warp_reserve`).
...
so the same code sieves both families and both signs.  THE ENGINE IS
INHERITED from lcm-ladders' v1 (itself linear-ladders' v4) with the
multiplier list swapped to k!; the kernel needed no change, and every
tuning constant below is that project's until OPTIMIZATION_LOG.md says it
was re-swept here.

UNIT SPACE.  Every candidate at every filter n >= 2 is a multiple of 6
(fladder_search.forced_unit: 2 and 3 are forced, and by Wilson nothing
else ever is), so the device sweeps x' = x / 6 with the kill sets
K'(q) = 6^-1 * K(q,n,F) mod q and 2 and 3 left out of the wheel.
Everything the device touches is in x'; `self.W` (the period), the
survivors and every bound check are in x, and `_collect` is the one place
the unit is multiplied back in.  `unit = 1` is the x-space engine, and the
gate battery runs it on wheels a dense CPU sieve can follow.  `assert_unit`
refuses a unit that is not forced at the filter, or not squarefree, which
is the one way this could thin the line.

ONE LINE, A WHEEL THAT GROWS.  The published term is x itself, so every
filter sweeps the same line: a find at filter n is the floor of filter
n + 1 and the cursor moves to it without re-denomination.  What changes
per filter is the KILL SET -- K(q,n) is a subset of K(q,n+1), saturating
at n = q - 1 -- and with it the residues kept per wheel level, the period
and the window width, all PLANNED per filter (`wheel_plan`, `plan_q2`) and
never stored.  The early filters (n = 11..15) have modelled medians so
close to the frontier that the period cap (`search_period_cap`) keeps
their wheels short; from n = 16 the full wheel fits.
...
CEILINGS, stated and enforced (CONVENTIONS.md "Numeric hygiene"):
  * x < k_ceil(n, F) = huntlib.ceiling.K_CEIL = 1e40 for every family and
    both signs (fladder_search, G10): a discovery is proved by a BLS75
    certificate on V - s = k!*x, and the ceiling is where that
...
period walk, both families), G8 (the wheel partitions the period: the
...
CLIPPED PERIOD 0), G13 (the production wheel constants of every filter),
...
and the clamp exactly at the bound), G20 (the planned wheel to 53 on the
wide record == the wheel to 47 on the narrow one over a whole 53-period),
G17
...
from fladder_reference import (FAMILIES, FOUND, KNOWN, family,  # noqa: E402
                               forbidden_k_residues, nforms, sign, w_count,
                               wheel_residues)
from fladder_search import (Q2_DEFAULT, CpuEngine, assert_unit,  # noqa: E402
...
#
# The unit is 6 at every filter (2 and 3 forced, nothing else ever), so
# unlike the lcm ladders the unit does not move -- but the kill sets do:
# K(q,n) grows with n until it saturates at n = q - 1, so the residues kept
# per level, the period W' and how many periods fit under 2^63 all change
# from filter to filter.  And the SEARCH changes: the modelled median for
# a(11) is 3e10 while a full wheel's period is 6e17, so `search_period_cap`
# holds the early filters to short wheels (a find is only known to be the
# least once its period closes, and a period longer than the search would
# over-sweep more than the wheel buys); from n = 16 the full wheel fits.
# `wheel_plan` computes it and `launch.py` never stores one.
#
...
def plan_q2(n, fam, unit, wheel, target=SURV_TARGET, ladder=Q2_LADDER):
    ...
    # does not divide the unit (fladder_reference G2b proves the count
...
# HOW LONG A SEGMENT MAY BE AGAINST THE SEARCH IT IS PART OF.
#
...
# therefore costs up to one segment of over-sweep -- v1 and v2 capped the
# PERIOD at a quarter of the median and let the window multiply it by up to
# 224, which at n = 16 made the first segment twelve medians long.  Since
# round 2 the launcher carries the classified line across a promotion, so
# an over-sweep is the next filter's work done at this filter's rate (60-75%
# as fast) rather than work thrown away; the expected over-sweep is half a
# segment, so a segment as long as the median costs ~15% of a median-time,
# against window gains of 1.2-1.5x and a wheel prime worth 1.2x.  The cap
# is therefore ONE median on the segment, and the wheel may be as long as
# admits a PV_MIN-period segment under it.  Per filter, like everything
# else here (n = 17: the wheel to 47 at 179 periods; n = 18: the wheel to
# 53 at 160; OPTIMIZATION_LOG.md round 3).
SEGMENT_MARGIN = 1.0


@_lru_cache(maxsize=None)
def search_segment_cap(n, fam, margin=SEGMENT_MARGIN):
    """The longest segment (in x) a plan at filter n may sweep, or None.
    ...
    """
    import fladder_model as _model
    from fladder_reference import KNOWN
    fam = family(fam)
...
def _wheel_value(n, fam, unit, top=WHEEL_TOP):
    ...
    unit = int(unit)
    return tuple((q, (q - w_count(q, n)) / q)
...
def wheel_plan(n, fam, unit, pv_min=PV_MIN, r1_max=R1_MAX, top=WHEEL_TOP,
    ...
    keep(q) = (q - w(q,n))/q, and the two are out of step here as well,
    though less wildly: at n = 17, 11 keeps 0.545 (only 5 of 1!..10! are
    distinct mod 11) where 13 keeps 0.308 and 17 keeps 0.353, and 31 keeps
    0.613 (12 of 17 factorials distinct) where 29 keeps 0.517.

    So the subset is chosen by VALUE DENSITY, -log(keep(q)) / log(q), taken
    greedily while the period still fits every bound.

    The bounds, all enforced:
      * W1 < 2^32, W2 < 2^32, W1*W2 < 2^63 (the kernel's arithmetic);
      * R2, R3 <= 65535 (they ride gridDim.y and .z), R1 <= r1_max;
      * (pv_min + 1) W' + q2 < 2^64 (the offset is a u64 and the Barrett
        tail's one conditional subtraction is exact to 2^64);
      * `max_segment`, the caller's statement of how long a SEGMENT may be
        against the search it is part of (search_segment_cap): the period
        may be at most max_segment / pv_min, so that a window of at least
        pv_min periods fits under the cap.  A find is only known to be the
        LEAST once its segment closes, so it costs up to one segment of
        over-sweep -- which is why n = 17 takes the wheel to 47 and n = 18
        the wheel to 53.
    """
    fam = family(fam)
    unit = int(unit)
    vals = _wheel_value(n, fam, unit, top)
    # the record bounds the PERIOD alone (v3): W' + q2 < 2^64, and the
    # kernel's arithmetic already needs W' < 2^63
    cap = (1 << 63) - 1
    if max_segment is not None:
        cap = min(cap, max(1, int(max_segment) // (unit * int(pv_min))))
    ...
    if best is None:
        raise ValueError(f"no admissible wheel at n = {n} of {fam} with unit "
                         f"{unit}: PV_MIN = {pv_min}, the 2^63 period bound "
                         f"and max_segment = {max_segment} leave nothing")
    return best[1]


@_lru_cache(maxsize=None)
def plan_pb(n, fam, unit, wheel, q2, max_segment=None, pb_max=None):
    """The window width (periods per segment) at filter n on `wheel`: the
    widest multiple of 32 up to PB_DEFAULT whose segment fits under
    `max_segment`, and -- where the narrow record admits at least
    WIDE_MIN_PV periods -- no wider than the narrow record admits, because
    past that the window is worth under 2% and the wide record costs 5-6%
    (OPTIMIZATION_LOG.md round 3).  At least PV_MIN."""
    unit = int(unit)
    Wp = 1
    for q in wheel:
        if unit % q:
    ...
    if maxp >= WIDE_MIN_PV and maxp < pv:
        # the narrow record clamps the live periods to what the u64 admits
        # (179 on the wheel to 47) inside a whole-word window, and a partial
        # last word is worth having (179 against 160 read 1.050 at n = 17,
        # round 2) -- so the word is rounded UP here: the engine's pv is
        # maxp, under the cap
        return max(PV_MIN, ((maxp + 31) // 32) * 32)
    # otherwise the cap (or PB_DEFAULT) binds and every period of the window
    # is live, so the window is rounded DOWN to keep the segment under it
    return max(PV_MIN, (pv // 32) * 32)
    ...
    def __init__(self, n, fam, p1=None, p2=None, p3=None,
        ...
        if planned:
            p1, p2, p3 = wheel_plan(
                ...
                max_segment=search_segment_cap(self.n, fam))
        ...
        for sig in QCAP_SIGMA_LADDER:
            ...
            ranked.append(((blocks, sig), caps, sig, tot, blocks))
        ranked.sort(key=lambda z: z[0], reverse=True)
        if not ranked:
            ranked = [((0, qcap_sigma), _caps(qcap_sigma), qcap_sigma, 0, 0)]
...
def g7_wheel_matches_oracle():
    ...
    for fam in fams:
        ...
        for n, p1, unit in ((11, 13, 6), (15, 13, 6), (16, 13, 6),
                            (17, 17, 6), (18, 13, 6), (1, 7, 2)):
        ...
        for n, p1, p2, unit in ((15, 11, 13, 6), (16, 13, 19, 6),
                                (18, 11, 13, 6)):
    ...
    for n, unit in ((11, 30), (17, 10), (15, 12), (1, 6)):
        try:
            wheel(n, "A177013", 13, unit=unit)
    ...
    return True, ("G7 ok: CRT-lifted wheel == oracle period walk at (n,p1) = "
                  ...
                  "== the oracle's divisibility on x = 6*x' at n = 11, 15, "
                  "16, 17, 18 (and x = 2*x' at n = 1) and recombines across "
                  "two levels; and the units 30, 10, 12 and 6-at-n=1 are "
                  "REFUSED, since nothing forces them there")
...
def g8_wheel_partitions_the_period():
    ...
    """
    from fladder_reference import w as w_formula
    cases = []
    for fam in FAMILIES:
    ...
    return True, (f"G8 ok: the wheel is exactly prod(q - w(q,n)) residues, "
                  ...
                  f"levels at every filter n = 11..18 of both families at the "
                  f"forced unit 6 -- the level split is re-derived per "
...
def g9_gpu_matches_cpu():
    ...
    """
    from fladder_search import k_proof
    cross_17 = k_proof(17, "A177013")          # 9.3e9, a crossing the hunt passes
    ceil = k_ceil(15, "A177013")               # 1e40, the enforced ceiling
    W19 = 9_699_690
    cases = (
        # fam, n, p1, p2, p3, q2, x_lo, span, unit
        #
        # q2 = 32 throughout; the wheel here keeps 0.4-0.6 of the line per
        # small prime, so a 1e7-4e7 window at q2 = 32 holds hundreds to
        # thousands of survivors, which is what makes a dense CPU sieve
        # affordable as the reference.
        ("A177013", 9, 13, None, None, 32, 2 * 10 ** 9, 2 * 10 ** 7, 1),
        ("A177014", 9, 13, None, None, 32, 2 * 10 ** 9, 2 * 10 ** 7, 1),
        ("A177013", 15, 13, None, None, 32, 10 ** 12, 10 ** 7, 1),
        ("A177014", 15, 13, 23, None, 32, 10 ** 12, 10 ** 7, 1),
        ("A177013", 15, 11, 19, 23, 32, 10 ** 12, 10 ** 7, 1),
        ("A177014", 17, 13, 23, None, 32, 9 * 10 ** 14, 2 * 10 ** 7, 1),
        ("A177013", 16, 13, 19, 23, 32, 9 * 10 ** 14, 2 * 10 ** 7, 1),
        ...
        # window noticing
        # (from x = 64: the floor is q2 - s = 33 here, because the smallest
        # value is the k = 1 form x + s, so the clip starts just above it)
        ("A177013", 9, 13, 19, None, 32, 64, W19 - 64, 1),
        ("A177014", 15, 13, 19, None, 32, 64, W19 - 64, 1),
        # UNIT SPACE: the device sweeps x' = x / 6 and the host multiplies
        # back; the CPU engine still marks the dense x line, so the forcing
        # lemma is checked here as well as the fold, at every filter of the
        # first night.
        ("A177013", 11, 13, 19, None, 32, 10 ** 12, 10 ** 7, 6),
        ("A177014", 15, 13, 19, 23, 32, 10 ** 12, 10 ** 7, 6),
        ("A177013", 16, 13, 19, 23, 32, 10 ** 12, 2 * 10 ** 7, 6),
        ("A177014", 16, 13, 23, None, 32, 10 ** 12, 2 * 10 ** 7, 6),
        ("A177013", 17, 13, 19, 23, 32, 10 ** 15, 2 * 10 ** 7, 6),
        ("A177014", 18, 13, 23, None, 32, 10 ** 15, 2 * 10 ** 7, 6),
        ("A177013", 19, 11, 17, 23, 32, 10 ** 15, 4 * 10 ** 7, 6),
        # ABOVE 2^64, and past the PROOF CROSSING at n = 17 (9.3e9), which
        # this campaign passes almost at once: both engines are Python ints
        # there.
        ("A177014", 17, 13, 19, 23, 32, cross_17 + 10 ** 12, 2 * 10 ** 7, 6),
        ("A177013", 15, 13, 19, None, 32, 10 ** 21, 10 ** 7, 6),
        # SUBSET WHEELS -- the shape the campaign actually runs.  A prime the
        # wheel declines (11 here, which keeps over half the line) must be
        # sieved instead, and the survivor stream must be the same as a
        # dense CPU sieve that knows nothing about either.
        ("A177013", 15, [5, 7, 13, 17, 19], [23, 29], None, 32,
         10 ** 12, 10 ** 7, 6),
        ("A177014", 17, [5, 7, 13, 17], [19, 23], [29], 32,
         10 ** 15, 2 * 10 ** 7, 6),
        # HARD AGAINST THE 1e40 CEILING, on both signs: the base is a
        # 133-bit Python int on both engines
        ("A177013", 15, 13, 19, 23, 32, ceil - 10 ** 9, 10 ** 7, 6),
        ("A177014", 16, 13, 19, 23, 32, ceil - 10 ** 9, 2 * 10 ** 7, 6),
    )
    total = 0
    ...
    return True, (f"G9 ok: GPU stream == CPU stream on {len(cases)} populated "
                  ...
                  f"wheels in x space AND in unit space (6), SUBSET wheels "
                  f"that decline 11 to the sieve, both families, filters "
                  f"n = 9 to 19, heights 2e9 -> {ceil:.3g} with windows ABOVE "
                  f"2^64 and past the n = 17 proof crossing ({cross_17:.3g}), "
                  f"two hard against the 1e40 ceiling on both signs; and "
                  f"period 0 clipped at the engine floor on both families")
...
def g13_production_wheel_constants():
    ...
    from fladder_reference import forced_primes, w as w_formula
    cases = [("A177013", 15, 23, 37, 47, 1), ("A177013", 16, 23, 37, 47, 1),
             ("A177014", 15, 23, 37, 47, 1), ("A177014", 16, 23, 37, None, 1)]
    ...
    return True, (f"G13 ok: the wheel constants (W1, W2a, W2b, W1^-1 and the "
                  ...
                  f"at every filter n = 11..18 of both families at unit 6; "
                  f"3000 sampled CRT residues per configuration recombine to "
                  f"all three levels, survive every wheel prime IN X SPACE "
                  f"(x = 6*x') and are multiples of every forced prime; "
...
def g14_engine_mechanisms():
    ...
    cases = (("A177013", 15, 23, 37, None, 65536, 1),
             ("A177013", 16, 23, 37, 47, 65536, 1),
             ("A177014", 15, 23, 37, 47, 65536, 1),
             ("A177013", 15, 23, 31, None, 4096, 1),
             # the unit wheels the campaigns run
             ("A177013", 15, 19, 31, 43, 131072, 6),
             ("A177013", 17, 19, 31, 43, 32768, 6),
             ("A177014", 15, 19, 31, 43, 131072, 6),
             ("A177014", 16, 23, 37, 47, 131072, 6),
             ("A177013", 18, 23, 37, 47, 32768, 6),
             ("A177014", 17, 19, 29, 41, 65536, 6))
    ...
    ref = GpuEngine(15, "A177013", p1=13, p2=23, p3=None, q2=128)
    tiny = GpuEngine(15, "A177013", p1=13, p2=23, p3=None, q2=128,
                     qcap_sigma=-1e9)
    ...
    prod = GpuEngine(17, "A177013", unit=6)
    ...
    return True, (f"G14 ok: the split A/C/D generation tables reproduce the "
                  ...
                  f"configurations including x-space (23,37,47] and unit-6 "
                  f"wheels at n = 15, 16, 17, 18; the compaction "
                  f"depths derive from the survival curve (A177013 n = 17: "
...
def g15_k_off_representation():
    ...
    eng = GpuEngine(15, "A177013", p1=13, p2=23, p3=None, q2=512)
    engb = GpuEngine(15, "A177013", p1=13, p2=23, p3=None, q2=512, pb=32,
                     tchunk=TPB_DEFAULT, nu=1, wide=True)
    engn = GpuEngine(15, "A177013", p1=13, p2=23, p3=None, q2=512, pb=32,
    ...
    # and in UNIT space, where the base the device folds is in x' and the
    # survivors come back multiplied: same invariance, and every survivor
    # a multiple of the unit that the x-space engine also keeps
    engu = GpuEngine(15, "A177013", p1=19, p2=23, p3=None, q2=512, unit=6)
    engub = GpuEngine(15, "A177013", p1=19, p2=23, p3=None, q2=512, unit=6,
    ...
    for i in np.random.default_rng(5).integers(0, len(engu.primes), 20).tolist():
        q = engu.primes[i]
        kr = killed_residues(q, 15, "A177013", 6)
    ...
    for i in rng.integers(0, len(eng.primes), 40).tolist():
        q = eng.primes[i]
        kr = killed_residues(q, 15, "A177013")
        row = pres[i]
    ...
    return True, (f"G15 ok: the survivor stream is INVARIANT under the launch "
                  ...
                  f"multiple of 6, with the residue lists the unit-space "
                  ...
                  f"prime and per round group, at six bases up to 1e30 -- so "
...
def g16_third_level_mechanisms():
    ...
    """
    n, fam, p1, p2, p3, q2 = 15, "A177013", 13, 17, 19, 128
...
def g17_unit_wheel_matches_k_wheel():
    ...
    THE WINDOW IS ONE PERIOD of the wheel to 43 at unit 6 -- 1.3e16 of x,
    about 4e11 candidates and a fraction of a second of device.

    The four wheels, all over the same window:
      * the wheel (..19],(19,31],(31,43] at unit 6 -- the shape the campaign
        runs from n = 15 on;
      * the SAME prime set split differently, (..23],(23,31],(31,43] -- so a
        CRT-lift bug that depends on where the levels break shows up;
      * a COARSER wheel, (..17],(17,29],(29,41], whose period is 43x
        shorter, so it covers the window in 43 periods and sieves 43 in its
      ...
      * an X-SPACE wheel (unit = 1) over primes to 43, whose period is the
        same 1.3e16 because the unit's primes are in it instead.
    """
    out = []
    for n, fam in ((15, "A177013"), (17, "A177014")):
        unit = forced_unit(n, fam)
    ...
    return True, ("G17 ok: four wheels, four arithmetics, one stream -- the "
                  "unit-6 wheel to 43, the same prime set split at a "
                  ...
                  + "; ".join(out) + " -- every survivor a multiple of 6, and "
                  "a 200-point sample passes the CPU engine's one-at-a-time "
                  "test")


def g18_every_opening_compiles_to_occupancy():
    """Every campaign opening (and the three filters after it), the two
    filters that cost the night (n = 16, 17), and every filter a RESUMED
    campaign runs or promotes into next, compiles to the occupancy the
    engine was tuned at, with the carveout pinned and the prefix tables
    inside their cap.
    ...
    for fam in FAMILIES:
        n0 = max(KNOWN[fam]) + 1
        # EVERY filter this campaign opens at over its first minutes (the
        # early filters are seconds each) and the two that cost the night,
        # each built the way the campaign builds it, with NO wheel named,
        # so what compiles here is exactly what runs.
        ns = list(range(n0, n0 + 4)) + [16, 17]
    ...
    return True, (f"G18 ok: all {len(rows)} campaign configurations (four "
                  f"filters past each family's frontier plus n = 16 and 17, "
                  f"and any resumed filter and the two after it, each PLANNED "
                  f"the way the campaign plans it) compile to >= "
...
def g19_reduction_bound_is_the_word():
    ...
    offsets pass 2^63.
      ...
      3. Device: the planned full wheel at n = 17, the first launch (a
    ...
    span = 256
    for pb in (64, 256):
        eng = GpuEngine(17, "A177013", p1=W47[0], p2=W47[1], p3=W47[2],
    ...
    try:
        ...
        try:
            GpuEngine(17, "A177013", p1=W47[0], p2=W47[1], p3=W47[2],
...
def g20_wide_wheel_matches_narrow_wheel():
    ...
    space at n = 18, on the narrow record and on the wide record (forced:
    ...
    # the planned 53-wheel: non-contiguous, and wide by the engine's choice
    cap = search_segment_cap(n, fam)
    lv = wheel_plan(n, fam, unit, max_segment=cap)
    top = max(_wheel_primes(*lv))
    if top != 53 or sorted(lv[0])[-1] < min(lv[1] + lv[2]):
        return False, (f"G20 FAIL: the plan at n = {n} is {lv} -- expected "
                       f"the wheel to 53 in a non-contiguous split")
    q2 = plan_q2(n, fam, unit, _wheel_primes(*lv))
    pb = plan_pb(n, fam, unit, _wheel_primes(*lv), q2, max_segment=cap)
    e53 = GpuEngine(n, fam, p1=lv[0], p2=lv[1], p3=lv[2], q2=q2, unit=unit,
                    pb=pb, seg_cap=pb)
    if not e53.wide or e53.seg_periods != pb or (pb + 1) * e53.Wp < 1 << 64:
        return False, (f"G20 FAIL: the planned 53-wheel came up "
                       f"{'wide' if e53.wide else 'narrow'} with "
                       f"{e53.seg_periods}-period segments (pb {pb})")
    return True, (f"G20 ok: the non-contiguous split {NC} at unit 6, n = {n}, "
                  f"returns the CPU engine's {len(want)} survivors over "
                  f"[{x_lo:.3g}, +{span:.3g}) on the narrow record AND on the "
                  f"wide record; the planned wheel to 53 at n = {n} is {lv} "
                  f"(non-contiguous, W' = {e53.Wp:.4g}: no u64 window admits "
                  f"it) at {pb} periods and comes up on the wide record by "
                  f"itself")
