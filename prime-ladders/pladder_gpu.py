"""pladder_gpu.py -- the GPU engine for A084700 and A084701.
...
the first kill.  That early exit is the whole performance argument: w(q,n,s)
is n out of every q for q > prime(n), so the first prime tested kills a
large fraction and the expected number of tests per candidate is small,
against the ~n*ln ln q2 marks per candidate a marking sieve of the same
depth would pay.

THIS IS square-ladders' ENGINE, AND WHY IT TRANSFERS.  The kernel knows
nothing about the forms: everything problem-specific enters through
`killed_residues(q, n, s)` -- the wheel tables, the per-prime masks and
residue lists, the CRT-combined prefix groups and the survival curve the
compaction points are derived from are all built from that one function.  square-ladders
kills k by { -(i^2)^-1 }, this project by { -s * prime(i)^-1 }, and the
same code sieves both.  What the kernel DOES inherit is the wheel's prime
set (2..23 flat, (23,37] and (37,47] as CRT levels), because the bounds
that stop the wheel at 47 -- W1 < 2^32 for a u32 residue table, W2 < 2^32
for the CRT multiplier -- are bounds on the PRIMES, not on the problem.

Two things are different here, and both are gifts:

  * FORCED DIVISIBILITY makes the residue tables tiny.  The first 14
    primes cover every nonzero residue mod 2, 3, 5, 7 and 11
    (pladder_reference G2c), so those five wheel primes contribute ONE
    surviving residue each instead of q - w.  At n = 14 the first-level
    table is 2,800 residues mod 223,092,870 against square-ladders'
    1,088,640, the second 7,344 and the third 27,720; the wheel leaves
    9e-7 of the line as candidates where square-ladders' left 1.3e-4.
    Same kernel, same wheel period of 6.15e17 -- and 140x fewer candidates
    per unit of line, which is what this project's line rate is made of.
  * THE SIGN COSTS NOTHING.  K(q,n,-1) = -K(q,n,+1), so the two families
    have identical table SIZES, identical survival curves and identical
    generated kernel source; only the table CONTENTS differ, and those are
    device arrays.  One compiled module serves both, and the module cache
    is keyed accordingly.

THE FACTORED WHEEL, THE ISSUE-BOUND TEST LOOP, THE (k, off) REPRESENTATION
and the compaction design are square-ladders' v5, unchanged; its
OPTIMIZATION_LOG.md holds the measurements that shaped each of them.  The
short version of what is in here:
  ...
  * the wheel is THREE tables combined per candidate by CRT
    ...
    8e7 residues is stored in tables of thousands;
  ...
  * divergence is compacted inside the CUDA block twice, into shared-memory
    queues sized from the ANALYTIC survival plus QCAP_SIGMA sigma, and the
    deep tail runs as its own kernel over a global queue; every queue's
    overflow is HARMLESS by construction (the candidate runs its tail on
    the spot), and G14/G16 force those paths and check the stream;
  * a candidate is the pair (base, off) with k = base + off: `base` is the
    ...
    `off < W * per_launch` is the only thing the device reduces.  The base
    is folded once per launch into the per-prime records (slot .w) and
    the group scalars, so no machine word bounds the search.  G15 proves
    the stream does not depend on where the launch base was put.

v2 (2026-09-02) CHANGED THREE THINGS, all measured in OPTIMIZATION_LOG.md:

  * the TAIL runs as compaction ROUNDS over global queues, each round a
    prime range from the survival curve, and a round too small to fill
    the device gives every item several LANES (primes strided across
    them, a vote per batch) -- the deep items are a latency chain, not a
    divergence problem, and that is what shortens the chain;
  * the tail's tables are a 4096-bit mask per prime (exact below 4096)
    plus a residue list, 3.3 MB in place of a 50 MB doubled bitmap;
  * round 2 is split by R2_SPLITS into in-block compaction rounds, and
    the host loop keeps a launch queued behind the one being read back.

v3 (2026-09-02) MOVED THE WHEEL INTO UNIT SPACE.  Every candidate at the
campaign filters is a multiple of a forced modulus (2310 from n = 14, 210
from n = 10: pladder_reference's forcing lemma), so the device sweeps
k' = k / unit with the kill sets K'(q) = unit^-1 * K(q,n,s) mod q
(pladder_search.killed_residues) and the unit's own primes left out of
the wheel.  That frees the u32 and 2^63 bounds that stopped the k-space
wheel at 47: (..31], (31,41], (41,53] at unit 2310 has W1 = 8.7e7, W2 =
1.6e8, a device period of 1.4e16 (3.26e19 of k) and 1.47x fewer
candidates per unit of line at n = 18 -- for the IDENTICAL survivor set
over any window (G17), because the sieve depth did not move.  Everything
the device touches is in k'; `self.W` (the period), the survivors and
every bound check are in k, and `_collect` is the one place the unit is
multiplied back in.  `unit = 1` is the k-space engine, unchanged, and
five of the six benchmark shapes still run it.  `assert_unit` refuses a
unit that is not forced at the filter, which is the one way this could
thin the line.  With the wheel came a re-sweep of the tuning constants
(CPT 64, SPB 16, LIT_SURV 0.28, K2_SURV 0.008 -- OPTIMIZATION_LOG.md v3).

ONE ADDITION (v1): A WINDOW MAY START INSIDE PERIOD ZERO.  A three-level period
is 6.15e17 of k line, and this project's frontiers -- a(13) = 1.6e14 and
a(11) = 3.9e12 -- sit INSIDE the first one, as do the modelled medians of
the next two terms of each family.  square-ladders never had to sweep
period 0 (its cursor was adopted from an earlier wheel), so its engine
refused any window whose first period began at or below the engine floor.
Here `sweep` and `survivors_j` take `k_min`: the device sieves the whole
period and the host DROPS every survivor below k_min before anyone sees
it.  That is exact -- a survivor above k_min is a survivor of a sieve
whose every prime is below k_min, so "q divides the value" still means
"composite" -- and it costs nothing on the device.  The floor check
becomes a check on k_min instead of on j0*W, and G9 pins the clipped
period-0 stream against the CPU engine.

CEILINGS, stated and enforced (CONVENTIONS.md "Numeric hygiene"):
  * k < k_ceil(n, s) = the PRIMALITY-PROOF VALIDITY BOUND of the family
    (pladder_search, G10).  For A084701 it is the deterministic
    Miller-Rabin bound rearranged, 9.0e22 at n = 12: every decision there
    is a proof.  For A084700 it is that bound on k ITSELF, 3.317e24: below
    the proof crossing k_proof(n, s) (5.4e22 at n = 18) the classification
    is a proof, above it a discovery is proved by a BLS75 certificate on
    N - 1 = prime(i)*k (launch.py certify_run), one level deep while every
    factor of k is under the bound.  No machine word appears in either.
  ...
  * W1 < 2^32, so the first-level table is u32; W2 < 2^32 and
    W1*W2 < 2^63 -- in unit space these are what let 53 in.
...
period walk, both signs), G8 (the wheel partitions the period: the count
is the formula and no kept residue is killed), G9 (GPU survivor stream ==
CPU survivor stream, bit for bit, on populated windows at several heights,
filters and both signs, one-, two- and three-level, including above 2^64
and a CLIPPED PERIOD 0), G13 (the production wheel constants), G14 (the
generation tables, the derived compaction depths, the CRT-combined prefix
and round-2 group tables, and the queue-overflow fallback), G15 (the
(k, off) representation: base-shift invariance, the tail's masks and
residue lists, and the folded group scalars), G16 (the third level: the global tail queue's
overflow, chunking, and the power-of-two queue index), G17 (the v3 unit
wheel returns the v2 wheel's identical survivors over a v2 period at the
live filter).  G7, G8, G9, G13, G14 and G15 each carry unit-space cases.
"""
...
from pladder_reference import (forbidden_k_residues,            # noqa: E402
                               wheel_residues)
from pladder_search import (Q2_DEFAULT, CpuEngine, assert_unit,  # noqa: E402
                            k_ceil, k_floor, killed_residues)

# THE K-SPACE WHEEL (v1/v2; unit = 1).  Kept as the defaults of this class
# because five of the six benchmark shapes and most of the gates run on
# it; the CAMPAIGN runs the unit wheel below.
...
P3_DEFAULT = 47
# THE UNIT WHEEL (v3; launch.py UNIT/P1/P2/P3).  The forcing lemma makes
# every candidate a multiple of 2310 from n = 14, so the device sweeps
# k' = k / 2310 and the primes of the unit leave the wheel: (..31] is then
# 13*17*19*23*29*31 = 8.7e7 (u32), (31, 41] and (41, 53] together
# 1.6e8 (u32), and 53 fits where in k space nothing did.  1.47x fewer
# candidates per unit of line at n = 18 for the same survivor set (G17);
# the measurements are in OPTIMIZATION_LOG.md v3.
TPB_DEFAULT = 128            # threads per block (re-swept on v2: 128 over 256 by 1.09x)
# TUNING CONSTANTS COME IN TWO SETS, one per wheel family, because the
# re-sweep on the unit wheel (OPTIMIZATION_LOG.md v3) moved every one of
# them and the k-space wheels -- which five of the six benchmark shapes
# and most gates still run -- lost 20-33% under the unit wheel's values.
# A constant tuned against one geometry is wrong for the other (Rule 3a),
# so each keeps the set measured on it; `unit > 1` selects.
#
...
SPB_DEFAULT = 8              # k-space wheels (v1/v2's measured value)
# On the unit wheel at CPT 64, 16 is 1.05x over 8 at n = 18 and 1.12x at
# the A084701 opening filter; 32 ties it at n = 18 but is 0.57x at
# n = 14, a shared-memory cliff.
SPB_UNIT = 16
# CANDIDATES PER THREAD, from which JPT (first-level residues per thread)
# follows as CPT // SPB.  32 is square-ladders' measured optimum and v1's;
# on the unit wheel at n = 18 (R1 = 94,080) 64 is 1.20x and 96, 128, 256
# are 0.20-0.26x -- a tile of 8,192 candidates is the edge of the shared
# memory the queues may take.
...
# curve: on the k-space wheel the first compaction where about 11% of
# candidates are left, the second where about 0.57% are.  A coarser wheel
# or a smaller filter kills faster, and a fixed count there would test
# past the point where anything is left to kill.
...
# on candidates a group or two had already killed -- and that was 30% of
# the sieve kernel.  Each split packs the survivors into a fresh shared
...
# stops testing.  Swept in OPTIMIZATION_LOG.md.
R2_SPLITS = (0.03,)
UNROLL = 4                   # independent Barrett chains in the queue tail
# Survivors buffered per launch.  Larger than square-ladders' 2^16 because
# the gate battery runs coarse wheels and shallow sieves where a launch of
# 2^30 candidates can keep hundreds of thousands; the engine RAISES on
# overflow rather than dropping, so this is a budget, not a bound.
HIT_CAP = 1 << 20
...
# constant (OPTIMIZATION.md 2.4); square-ladders measured it flat to 2.6%.
...
# rare-and-deep -- a mean of ~40 tests each but a warp-max near 440, an
# 11x divergence tax -- so the tail sweeps its queue in rounds over prime
# ranges chosen from the survival curve: every round starts with every
# lane alive, and a lane that dies waits at most to the end of its round.
# TAIL_ROUND_DROP is the survival a round is allowed to lose before the
# survivors are compacted into the next queue (0.5: about 14 rounds at
# q2 = 65536).  Each round is one kernel launch over a global queue with
# one item per thread and ONE global atomic per block for the push.
TAIL_ROUND_DROP = 0.5
TAIL_TPB = 256
...
# chain of dependent loads on an otherwise empty device, and six such
# rounds were 80% of the tail for 0.4% of its items.  So a round whose
...
# q one of the w(q,n,s) <= n residues q kills".  v1 answered it from a
# doubled bitmap of 2q bits per prime -- 50 MB at q2 = 65536, gathered at
# random, and the tail's cost was those gathers' L2 sector traffic.  Now
# each prime carries a MASK of MASK_BITS bits over r mod MASK_BITS: for
# q <= MASK_BITS that IS the exact kill pattern (99.6% of tail items die
# below 4096), and above it a clear bit is still a proof of survival while
# a set bit -- about n in MASK_BITS -- goes to the exact residue LIST.
# 3.3 MB of masks at 4096 bits, L2-resident with room to spare.
MASK_BITS = 4096
NRES_MAX = 32                # residue-list slots; the filter n may not exceed it
...
# exceeds them pays a synchronous read.  Production is ~550 per launch.
PRE_COPY = 1 << 13
...
def wheel(n, s, p1, lo=1, unit=1):
    ...
    nothing.  That is the whole trade the unit buys -- 2, 3, 5, 7, 11 leave
    the wheel, and 29, 31 and 53 come in under the same u32 / 2^63 bounds.
    ...
    gate.  Here the tables are small (forced divisibility), but a flat
    wheel to 47 at n = 14 would still be 3.7e8 residues.
    """
    unit = int(unit)
    qs = [q for q in primerange(lo + 1, p1 + 1) if unit % q]
    W, count = 1, 1
    for q in qs:
        count *= q - len(killed_residues(q, n, s, unit))
    if count > RES_MAX:
        raise ValueError(
    ...
    for q in qs:
        bad = np.array(killed_residues(q, n, s, unit), dtype=np.int64)
...
def lit_groups(primes, nlit, budget=LIT_GROUP_MAX, start=0):
    ...
    "Killed by 53 or by 59" is a function of k mod (53*59) alone, so a
...
def lit_prefix(n, s, primes, groups, table="gbits", indent=12, pname="gp",
               hoist=None, unit=1):
    ...
    for gi, g in enumerate(groups):
        ...
        if Q < LIT_INLINE_Q:
            mask = 0
            for u in killed_residues(qs[0], n, s, unit):
                mask |= 1 << u
...
/* m = A[t] + C[s] reduced mod W2, taken from D = W2 - C rather than from
   ...
   third wheel level (W2 is then 2.76e9).  A - D never overflows; when it
   borrows, adding W2 back wraps to exactly A + C. */
...
class GpuEngine:
    ...
    def __init__(self, n, s, p1=P1_DEFAULT, p2=P2_DEFAULT, p3=P3_DEFAULT,
                 q2=Q2_DEFAULT,
        ...
        if self.s not in (1, -1):
            raise ValueError(f"the sign must be +1 or -1, not {s}")
        # THE UNIT (v3): the device sweeps k' = k / unit.  Every wheel,
        # table, fold and offset below is in k' space; the period the
        # HOST sees (self.W), the survivors it receives and every bound it
        # checks are in k.  `assert_unit` refuses a unit that is not forced
        # at this filter -- the one way this could thin the line.
        self.unit = assert_unit(self.n, self.s, unit)
        ...
        if self.n > NRES_MAX:
            raise ValueError(
                f"a filter of n = {n} conditions can kill up to {n} residues "
                f"per prime, and the tail's residue list holds NRES_MAX = "
                f"{NRES_MAX} slots: a longer ladder is a new engine version "
                f"with a wider record, not a constant to raise")
        self.p1, self.p2, self.p3 = p1, p2, p3
        self.q2, self.tpb = q2, tpb
        s = self.s

        self.W1, res1 = wheel(n, s, p1, unit=self.unit)
        if self.W1 >= 1 << 32:
            raise ValueError(
        ...
        if p2 and p2 > p1:
            W2a, res2 = wheel(n, s, p2, lo=p1, unit=self.unit)
        ...
        if p3 and p3 > wheel_top and self.R2 > 1:
            W2b, res3 = wheel(n, s, p3, lo=wheel_top, unit=self.unit)
        ...
        if self.R2 > 1:
            if self.W2 >= 1 << 32 or self.W1 * self.W2 >= 1 << 63:
                raise ValueError(
                    ...
                    f"this is where the wheel stops (the last prime that "
                    f"fits is 47)")
        ...
        for q in self.primes:
            ...
            surv *= 1.0 - len(killed_residues(q, n, s, self.unit)) / q
        self.surv.append(surv)
        ...
        self.nres = 16 if n <= 16 else NRES_MAX
        ...
        for i, q in enumerate(self.primes):
            kr = killed_residues(q, n, s, self.unit)
            if len(kr) > self.nres:
...
{body}
    ...
    const int nq{r} = min(qn{r}, Q{r}CAP);""")
        ...
        # pattern is either a device array or a launch scalar, and w(q,n,s)
        # -- hence the survival curve, the compaction depths and the group
        # moduli -- is sign-independent (pladder_reference G2c).  So the
        # two families share one compiled module, and the cache key says
        # so by omitting s.  G9 runs both signs through it.
        key = (n, self.unit, self.W1, self.W2, q2, tpb, self.jpt, self.spb,
    ...
    def config(self):
        return {"n": self.n, "s": self.s, "p1": self.p1, "p2": self.p2,
                "p3": self.p3, "q2": self.q2, "unit": self.unit,
                "W": int(self.W), "Wp": int(self.Wp),
    ...
    def _check_window(self, j0, j1, k_min):
        ...
        Without `k_min` the window must start above the engine floor, as
        square-ladders' engine required.  With it the window may start
        anywhere -- period 0 included -- and every survivor below k_min is
        dropped on the host; what is required is then that k_min itself is
        above the floor, since that is where the sieve's argument holds.
        Returns the effective clip (k_min, or 0 when none is needed).
        """
        ceil = k_ceil(self.n, self.s)
        if j1 * self.W > ceil:
        ...
        if k_min <= floor:
            raise ValueError(
                f"k_min = {k_min} is at or below max(K_FLOOR, q2) = {floor}: "
                f"survivors there are not survivors, so the clip may not "
                f"start under the engine floor")
        return k_min
...
def g7_wheel_matches_oracle():
    ...
    for both signs; and the CRT recombination reproduces the one-level
    wheel exactly."""
    for s in (+1, -1):
        for n, p1 in ((5, 7), (10, 11), (14, 11), (14, 13), (16, 13)):
            W, res = wheel(n, s, p1)
            Wo, reso = wheel_residues(n, s, p1)
            if W != Wo or list(res) != list(reso):
                return False, (f"G7 FAIL: s={s:+d} n={n} p1={p1}: lifted "
                               f"{len(res)} residues mod {W}, oracle "
                               f"{len(reso)} mod {Wo}")
        for n, p1, p2 in ((14, 7, 13), (14, 11, 17), (10, 7, 11)):
            W1, r1 = wheel(n, s, p1)
            W2, r2 = wheel(n, s, p2, lo=p1)
            inv = pow(W1 % W2, -1, W2)
            got = sorted(int(a) + W1 * (((int(b) - int(a)) * inv) % W2)
                         for a in r1 for b in r2)
            Wf, ref_res = wheel(n, s, p2)
            if W1 * W2 != Wf or got != [int(v) for v in ref_res]:
                return False, (f"G7 FAIL: s={s:+d} CRT recombination at "
                               f"n={n} ({p1},{p2}]: {len(got)} vs "
                               f"{len(ref_res)}")
        ...
        for n, p1, unit in ((14, 19, 2310), (18, 19, 2310), (10, 13, 210),
                            (8, 11, 30)):
            W, res = wheel(n, s, p1, unit=unit)
            qs = [q for q in primerange(2, p1 + 1) if unit % q]
            Wo = 1
            for q in qs:
                Wo *= q
            killed = {q: forbidden_k_residues(q, n, s) for q in qs}
            reso = [r for r in range(Wo)
                    if all((unit * r) % q not in killed[q] for q in qs)]
            if W != Wo or list(res) != reso:
                return False, (f"G7 FAIL: s={s:+d} n={n} p1={p1} unit={unit}: "
                               f"lifted {len(res)} residues mod {W}, oracle "
                               f"walk {len(reso)} mod {Wo}")
        for n, p1, p2, unit in ((14, 13, 19, 2310), (10, 11, 13, 210)):
            W1, r1 = wheel(n, s, p1, unit=unit)
            W2, r2 = wheel(n, s, p2, lo=p1, unit=unit)
            inv = pow(W1 % W2, -1, W2)
            got = sorted(int(a) + W1 * (((int(b) - int(a)) * inv) % W2)
                         for a in r1 for b in r2)
            Wf, ref_res = wheel(n, s, p2, unit=unit)
            if W1 * W2 != Wf or got != [int(v) for v in ref_res]:
                return False, (f"G7 FAIL: s={s:+d} CRT recombination with "
                               f"unit {unit} at n={n} ({p1},{p2}]: "
                               f"{len(got)} vs {len(ref_res)}")
    return True, ("G7 ok: CRT-lifted wheel == oracle period walk at "
                  "(n,p1) = (5,7), (10,11), (14,11), (14,13), (16,13) for "
                  "both signs; the two-level CRT recombination == the "
                  "one-level wheel at three (p1,p2] splits, both signs; in "
                  "unit space the k' wheel == the oracle's divisibility on "
                  "k = unit*k' at (n,p1,unit) = (14,19,2310), (18,19,2310), "
                  "(10,13,210), (8,11,30) and recombines across two levels, "
                  "both signs")
...
def g8_wheel_partitions_the_period():
    ...
    """
    from pladder_reference import w as w_formula
    for s in (+1, -1):
        for n, p1, lo, unit in ((14, 13, 1, 1), (14, 23, 1, 1), (12, 23, 1, 1),
                                (16, 19, 1, 1), (14, 37, 23, 1),
                                (14, 31, 23, 1), (16, 47, 37, 1),
                                (12, 47, 37, 1),
                                # the v3 production levels, in unit space
                                (14, 31, 1, 2310), (18, 31, 1, 2310),
                                (14, 41, 31, 2310), (14, 53, 41, 2310),
                                (18, 53, 41, 2310), (12, 31, 1, 210),
                                (12, 53, 41, 210)):
            W, res = wheel(n, s, p1, lo=lo, unit=unit)
            qs = [q for q in primerange(lo + 1, p1 + 1) if unit % q]
            want = 1
            for q in qs:
                want *= q - w_formula(q, n, s)
            if res.size != want:
                return False, (f"G8 FAIL: s={s:+d} n={n} ({lo},{p1}] unit "
                               f"{unit}: {res.size} residues, formula says "
                               f"{want}")
            if len(set(res.tolist())) != res.size:
                return False, (f"G8 FAIL: s={s:+d} n={n} ({lo},{p1}]: "
                               f"duplicate residues")
            for q in qs:
                bad = set(killed_residues(q, n, s, unit))
                if np.isin(res % q,
                           np.array(sorted(bad), dtype=np.int64)).any():
                    return False, (f"G8 FAIL: s={s:+d} n={n} ({lo},{p1}] unit "
                                   f"{unit}: a kept residue is killed by "
                                   f"q={q}")
    return True, ("G8 ok: the wheel is exactly prod(q - w(q,n)) residues, "
                  "duplicate-free, none of them killed by a wheel prime, "
                  "for both signs at (n,p1) = (14,13), (14,23), (12,23), "
                  "(16,19), at the second- and third-level ranges "
                  "(23,37], (23,31], (37,47], and at the v3 production "
                  "levels in unit space -- (..31], (31,41], (41,53] at unit "
                  "2310 for n = 14, 18 and at unit 210 for n = 12")


def g9_gpu_matches_cpu():
    """GPU survivor stream == CPU survivor stream, bit for bit.

    Populated windows at several filters and heights, both signs, one-,
    two- and three-level wheels, the top windows hard against the enforced
    ceiling and ABOVE 2^64 (the CPU side is Python ints all the way up),
    and a CLIPPED PERIOD 0 -- the window this project's campaign actually
    starts in, which square-ladders' engine could not sweep.  An
    empty-vs-empty comparison is vacuous and is refused.

    The windows are wide because survivors are sparse: forced divisibility
    alone leaves one k in 2310 at n = 14.
    """
    ceil14 = k_ceil(14, +1)
    ceil12m = k_ceil(12, -1)
    W23 = 223_092_870
    cases = (
        # s, n, p1, p2, p3, q2, k_lo, span, unit
        (+1, 10, 13, None, None, 256, 2 * 10 ** 9, 4 * 10 ** 7, 1),
        (+1, 14, 13, None, None, 128, 10 ** 12, 10 ** 8, 1),
        (-1, 12, 17, None, None, 128, 9 * 10 ** 14, 10 ** 8, 1),
        (+1, 14, 23, None, None, 64, ceil14 - 3 * W23, 3 * 10 ** 8, 1),
        (+1, 14, 13, 23, None, 128, 10 ** 12, 10 ** 8, 1),
        (-1, 14, 13, 17, None, 128, 9 * 10 ** 14, 10 ** 8, 1),
        (+1, 14, 11, 23, None, 64, ceil14 - 3 * W23, 3 * 10 ** 8, 1),
        (+1, 10, 11, 13, 17, 256, 2 * 10 ** 9, 4 * 10 ** 7, 1),
        (-1, 12, 11, 13, 17, 64, 10 ** 12, 10 ** 8, 1),
        (+1, 14, 13, 17, 19, 128, 9 * 10 ** 14, 10 ** 8, 1),
        (+1, 14, 17, 19, 23, 64, ceil14 - 3 * W23, 3 * 10 ** 8, 1),
        # PERIOD 0, clipped at the engine floor: the campaign's first window
        (+1, 10, 13, 23, None, 128, k_floor(128) + 1,
         W23 - k_floor(128) - 1, 1),
        (-1, 12, 13, 23, None, 128, k_floor(128) + 1,
         W23 - k_floor(128) - 1, 1),
        # UNIT SPACE (v3): the device sweeps k' = k / unit and the host
        # multiplies back; the CPU engine still marks the dense k line, so
        # the forcing lemma is checked here as well as the fold -- one-,
        # two- and three-level unit wheels, both signs, both units, at
        # heights up to the A084700 ceiling (3.3e24, above 2^64), the
        # A084701 ceiling, and period 0 clipped at the floor
        (+1, 10, 13, None, None, 128, 2 * 10 ** 9, 4 * 10 ** 7, 210),
        (+1, 14, 19, 23, 29, 128, 10 ** 12, 10 ** 8, 2310),
        (+1, 14, 19, 23, None, 64, ceil14 - 5 * 10 ** 9, 3 * 10 ** 8, 2310),
        # (a unit period at (..31] is 2.0e11, so the window sits 1e12 under
        # the ceiling: five of its periods, and still above 2^64)
        (+1, 18, 19, 29, 31, 64, ceil14 - 10 ** 12, 10 ** 9, 2310),
        (-1, 12, 13, 17, 19, 128, 9 * 10 ** 14, 10 ** 8, 210),
        (-1, 12, 13, 19, None, 64, ceil12m - 10 ** 12, 3 * 10 ** 8, 210),
        (-1, 14, 19, 23, 29, 128, 10 ** 12, 10 ** 8, 2310),
        (+1, 14, 13, 17, 19, 128, k_floor(128) + 1,
         2310 * 4199 - k_floor(128) - 1, 2310),
    )
    total = 0
    for s, n, p1, p2, p3, q2, k_lo, span, unit in cases:
        eng = GpuEngine(n, s, p1=p1, p2=p2, p3=p3, q2=q2, unit=unit)
        got = eng.survivors_k(k_lo, k_lo + span)
        cpu = CpuEngine(n, s, q2=q2)
        want = [k for c in cpu.survivors(k_lo, k_lo + span) for k in c]
        if got != want:
            gs, ws = set(got), set(want)
            return False, (f"G9 FAIL: s={s:+d} n={n} p1={p1} p2={p2} p3={p3} "
    ...
    return True, (f"G9 ok: GPU stream == CPU stream on {len(cases)} populated "
                  ...
                  f"wheels in k space AND in unit space (210, 2310), both "
                  f"signs, filters n = 10, 12, 14, 18, heights 2e9 -> "
                  f"{ceil14:.3g} with the top windows ABOVE 2^64, and period "
                  f"0 clipped at the engine floor on both")
...
def g13_production_wheel_constants():
    ...
    definition, for the filters both families will actually run.
    """
    rng = np.random.default_rng(20260902)
    from pladder_reference import w as w_formula
    for s, n, p1, p2, p3, unit in (
            (+1, 14, 23, 37, 47, 1), (+1, 15, 23, 37, 47, 1),
            (+1, 16, 23, 37, 47, 1), (+1, 17, 23, 37, None, 1),
            (-1, 12, 23, 37, 47, 1), (-1, 14, 23, 37, 47, 1),
            # v3: the unit wheels the campaigns run -- 2, 3, 5, 7, 11 out,
            # 29, 31 and 53 in, under the same u32 and 2^63 bounds
            (+1, 14, 31, 41, 53, 2310), (+1, 17, 31, 41, 53, 2310),
            (+1, 18, 31, 41, 53, 2310), (+1, 19, 31, 41, 53, 2310),
            (-1, 12, 31, 41, 53, 210), (-1, 14, 31, 41, 53, 210)):
        W1, r1 = wheel(n, s, p1, unit=unit)
        W2a, r2 = wheel(n, s, p2, lo=p1, unit=unit)
        if p3:
            W2b, r3 = wheel(n, s, p3, lo=p2, unit=unit)
        ...
        if W2 >= 1 << 32 or W1 >= 1 << 32 or W1 * W2 >= 1 << 63:
            return False, (f"G13 FAIL: n={n} ({p1},{p2},{p3}] unit {unit}: "
                           f"W1 = {W1}, W2 = {W2} exceed the u32 / 2^63 "
                           f"bounds the kernel's arithmetic rests on")
        inv = pow(W1 % W2, -1, W2)
        if W1 % W2 * inv % W2 != 1:
        ...
        if r1.size * r2.size * r3.size != want:
            return False, (f"G13 FAIL: s={s:+d} n={n} ({p1},{p2},{p3}] unit "
        ...
        killed = {q: set(killed_residues(q, n, s))
        ...
        for t, sx, u in zip(ts.tolist(), ss.tolist(), us.tolist()):
            ...
            for q, bad in killed.items():
                if (unit * x) % q in bad:
                    return False, (f"G13 FAIL: s={s:+d} n={n} "
                                   f"({p1},{p2},{p3}] unit {unit}: k = "
                                   f"{unit}*{x} is killed by q={q}")
            forced = 2310 if n >= 14 else 210     # the forcing lemma, G2c
            if (unit * x) % forced:
                return False, (f"G13 FAIL: n={n}: a wheel residue k = "
    ...
    return True, ("G13 ok: the wheel constants (W1, W2a, W2b, W1^-1 and the "
                  "two CRT lifts) are exact at the v2 configurations "
                  "(23,37,47] for n = 14, 15, 16 (A084700) and n = 12, 14 "
                  "(A084701), (23,37] at n = 17, and at the v3 unit wheels "
                  "(..31],(31,41],(41,53] at unit 2310 for n = 14, 17, 18, "
                  "19 and unit 210 for n = 12, 14; 3000 sampled CRT residues "
                  "per configuration recombine to all three levels, survive "
                  "every wheel prime IN K SPACE (k = unit*x) and are "
                  "multiples of the forced modulus; W1, W2 under 2^32 and "
                  "W1*W2 under 2^63")
...
def g14_engine_mechanisms():
    ...
    cases = ((+1, 14, 23, 37, None, 65536, 1), (+1, 16, 23, 37, 47, 65536, 1),
             (-1, 12, 23, 37, 47, 65536, 1), (+1, 14, 23, 31, None, 4096, 1),
             # v3: the unit wheels the two campaigns run
             (+1, 18, 31, 41, 53, 65536, 2310),
             (-1, 12, 31, 41, 53, 65536, 210))
    for s, n, p1, p2, p3, q2, unit in cases:
        W1, r1 = wheel(n, s, p1, unit=unit)
        W2a, r2 = wheel(n, s, p2, lo=p1, unit=unit)
        if p3:
            W2b, r3 = wheel(n, s, p3, lo=p2, unit=unit)
        ...
        if any(unit % q == 0 for q in primes):
            return False, (f"G14 FAIL: n={n} unit {unit}: a prime of the "
                           f"unit is in the sieve")
        ...
        for tag, groups, table_src, budget, lo, hi in (
                ("prefix", eng.groups,
                 lit_prefix(n, s, primes, eng.groups, unit=unit),
                 LIT_GROUP_MAX, 0, eng.lit),
                ("round 2", eng.groups2,
                 lit_prefix(n, s, primes, eng.groups2, table="g2bits",
                            unit=unit),
            ...
            for g in groups:
                ...
                if Q > budget:
                    return False, (f"G14 FAIL: n={n}: {tag} group {qs} has "
                                   f"modulus {Q} over the {budget} budget")
                if f"* {Q}u;" not in src or \
                        f"__umul64hi(off, {(1 << 64) // Q}ULL)" not in src:
                    return False, (f"G14 FAIL: n={n}: modulus {Q} or its "
                                   f"magic is not baked into the {tag} source")
                want = np.zeros(Q, dtype=bool)
                for q in qs:
                    bad = np.array(killed_residues(q, n, s, unit),
                                   dtype=np.int64)
    ...
    ref = GpuEngine(14, +1, p1=13, p2=23, p3=None, q2=128)
    tiny = GpuEngine(14, +1, p1=13, p2=23, p3=None, q2=128,
                     qcap_sigma=-1e9)
    ...
    prod = GpuEngine(18, +1, p1=31, p2=41, p3=53, unit=2310)
    ...
    return True, (f"G14 ok: the split A/C/D generation tables reproduce the "
                  f"one-table CRT on 12000 sampled triples at six "
                  f"configurations including the v2 (23,37,47] at n = 14, "
                  f"16 and n = 12 (A084701) and the v3 unit wheels "
                  f"(..31],(31,41],(41,53] at n = 18 (unit 2310) and n = 12 "
                  f"(unit 210); the compaction depths derive from the "
                  f"survival curve (production n = 18: LIT={prod.lit}, "
                  f"K2={prod.k2}); the "
                  f"CRT-combined prefix and round-2 groups cover their "
                  f"prime ranges exactly once in order, stay inside their "
                  f"budgets, and every group's table agrees with "
                  f"killed_residues on EVERY residue of its modulus; "
                  f"production queues hold {prod.q1cap} and {prod.q2cap} of "
                  f"a {prod.tile}-candidate block; and with the queues "
                  f"forced to 32 the survivor stream is IDENTICAL "
                  f"({len(a)} survivors)")
...
def g15_k_off_representation():
    ...
    """
    eng = GpuEngine(14, -1, p1=13, p2=23, p3=None, q2=512)
    j0 = eng.j_of(10 ** 12)
    eng.per_launch = 1
    one = eng.survivors_j(j0, j0 + 60)
    eng.per_launch = 6
    many = eng.survivors_j(j0, j0 + 60)
    if not one:
    ...
    if one != many:
        return False, (f"G15 FAIL: the stream depends on the launch base: "
                       f"{len(one)} survivors in 60 launches vs {len(many)} "
                       f"in 10 -- the fold is not base-invariant")
    ...
    engu = GpuEngine(14, -1, p1=19, p2=23, p3=None, q2=512, unit=2310)
    ju = engu.j_of(10 ** 12)
    engu.per_launch = 1
    oneu = engu.survivors_j(ju, ju + 60)
    engu.per_launch = 6
    manyu = engu.survivors_j(ju, ju + 60)
    if not oneu:
        return False, "G15 FAIL: the unit base-shift window is empty -- vacuous"
    if oneu != manyu:
        return False, (f"G15 FAIL: with unit 2310 the stream depends on the "
                       f"launch base: {len(oneu)} vs {len(manyu)}")
    if any(k % 2310 for k in oneu):
    ...
    for i in np.random.default_rng(5).integers(0, len(engu.primes), 20).tolist():
        q = engu.primes[i]
        kr = killed_residues(q, 14, -1, 2310)
        if sorted(presu[i][:len(kr)].tolist()) != kr:
            return False, (f"G15 FAIL: with unit 2310, prime {q}'s residue "
    ...
    for base in (0, eng.W, 10 ** 12, 2 ** 64 + 12345, k_ceil(14, -1) - eng.W,
                 10 ** 30):
        base -= base % eng.W
    ...
    return True, (f"G15 ok: the survivor stream is INVARIANT under the launch "
                  f"base ({len(one)} survivors, 60 launches vs 10; and "
                  f"{len(oneu)} in unit space, every one a multiple of 2310, "
...
def g16_third_level_mechanisms():
    ...
    """
    n, s, p1, p2, p3, q2 = 14, +1, 13, 17, 19, 128
    lo, span = 9 * 10 ** 14, 4 * 10 ** 9
    ref = GpuEngine(n, s, p1=p1, p2=p2, p3=p3, q2=q2)
    want = ref.survivors_k(lo, lo + span)
    if not want:
        return False, "G16 FAIL: the drill window is empty -- vacuous"

    flood = GpuEngine(n, s, p1=p1, p2=p2, p3=p3, q2=q2)
    flood.q3cap = 0
    ...
    for nu in (1, 2, 3):
        e = GpuEngine(n, s, p1=p1, p2=p2, p3=p3, q2=q2, nu=nu)
        if e.nu != min(nu, e.R3):
    ...
    for bad in (dict(tpb=192), dict(spb=6, cpt=24)):
        try:
            GpuEngine(n, s, p1=p1, p2=p2, p3=p3, q2=q2, **bad)
        except ValueError:
            pass
        else:
            if bad.get("tpb"):
...
def g17_unit_wheel_matches_k_wheel():
    """The v3 production wheel == the v2 production wheel, on the line.

    G9 proves the unit machinery on wheels a dense CPU sieve can follow;
    this pins the wheel the campaign actually runs -- (..31],(31,41],(41,53]
    at unit 2310, a period of 3.26e19 -- against the v2 wheel
    (23],(37],(47] over one v2 period (6.15e17) at the live filter n = 18,
    at the height the campaign resumes from.  Two wheels enumerating the
    same candidates by different arithmetic must return the identical
    survivor stream, and every survivor must also pass the CPU engine's
    one-at-a-time k-space test at the campaign's sieve depth.  Coverage is
    what a fingerprint cannot see and a cursor re-denomination rests on,
    so this is the gate the v2 -> v3 adoption stands on.
    """
    n, s = 18, +1
    v2 = GpuEngine(n, s, p1=23, p2=37, p3=47, q2=Q2_DEFAULT)
    v3 = GpuEngine(n, s, p1=31, p2=41, p3=53, q2=Q2_DEFAULT, unit=2310)
    j2 = 88434                       # the period the first campaign ended on
    lo, hi = j2 * v2.W, (j2 + 1) * v2.W
    a = v2.survivors_j(j2, j2 + 1)
    b = v3.survivors_k(lo, hi)
    if not a:
        return False, "G17 FAIL: the v2 period is empty -- vacuous"
    if a != b:
        sa, sb = set(a), set(b)
        return False, (f"G17 FAIL: over [{lo:.4g}, {hi:.4g}) the v2 wheel "
                       f"keeps {len(a)} survivors and the v3 unit wheel "
                       f"{len(b)}: diff {sorted(sa ^ sb)[:4]}")
    cpu = CpuEngine(n, s, q2=Q2_DEFAULT)
    if not all(cpu.survives(k) for k in b):
        return False, "G17 FAIL: a survivor fails the CPU engine's k-space test"
    if any(k % 2310 for k in b):
        return False, "G17 FAIL: a survivor is not a multiple of 2310"
    return True, (f"G17 ok: the v3 unit wheel (..31],(31,41],(41,53] at unit "
                  f"2310 returns the IDENTICAL {len(a)} survivors as the v2 "
                  f"wheel (23],(37],(47] over v2 period {j2} "
                  f"[{lo:.4g}, {hi:.4g}) at n = 18, every one a multiple of "
                  f"2310 that the CPU engine's k-space test also keeps -- "
                  f"the v3 wheel's density is {v2.density() / v3.density():.3f}x "
                  f"thinner for the same line")
