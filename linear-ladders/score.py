...
Six shapes, because one configuration is not a benchmark:
  ...
  SCORE     A088250, n = 15, the production unit wheel (..31],(31,41],(41,53]
            at unit 30030, sieve 65536, from period 1 (k = 3.3e19) -- the
            configuration the headline campaign OPENS in, and the row to
            read for opening throughput.  Denominated in kernel LAUNCHES
            rather than wheel periods because one period is 3.26e19 of k
            line; 64 launches is 64 x 130 of the 34,048 third-level
            residues a period holds and just as reproducible a set of
            candidates.
  SCORE17   the same wheel at n = 17 -- the filter the A088250 campaign
            ...
            within minutes; a(17)'s median is 1.7e24, hours) -- over TWO
            whole periods from period 1, because a period there is only 26
            launches.  The tables are a tenth the size of n = 15's and the
            line rate 13x, so this is the row to read for the live hunt.
  SCOREM    A125838, n = 15, the unit wheel at unit 30030 -- a -1 family's
            ...
            2.9x denser and the line rate a third.  The other w-class's
...
All six were frozen on 2026-09-03 on the v1 engine.  A deliberate coverage
change -- a new wheel, a new sieve depth -- legitimately moves a
fingerprint: update it in the same commit and say why in
OPTIMIZATION_LOG.md.

Wall clock: about 2.5 min, of which the gates are most.
"""
...
# period j0.
SHAPES = [
    ("SCORE",   "A088250", 15, 31,   41,   53, 65536,           1, None,   64,
     16439, 37540909221740710330, 30030),
    ("SCORE17", "A088250", 17, 31,   41,   53, 65536,           1,    2, None,
     1601, 28544938847964392360, 30030),
    ("SCOREM",  "A125838", 15, 31,   41,   53, 65536,           1, None,   64,
     46093, 8927501652700787704, 30030),
