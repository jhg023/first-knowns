...
for the run-breaking composite. Evidence JSONs for the **first
occurrences** in `evidence/`; everything else the sweep met (run-17s and
run-18s after a(17)/a(18) landed, the run-13..16 ladder) is the census
and is reported here as **counts per run length**, the way the launcher's
`[STATUS]` line reports it. Per-value census files and the near-miss
`.jsonl` written by earlier legs were retired on 2026-08-18 under the
repo-wide convention (CONVENTIONS.md, "The census is counted, not
narrated"); the git history before commit `3d01f95` has them.
...
- Evidence: census, not a first occurrence — its per-value file was
  retired 2026-08-18 (git history before `3d01f95`); it was verified
  three ways at the time.
...
11,660,916,128,654,455,667. Each was verified three ways with a factor
witness when found; only a(17)'s file remains in `evidence/` (census is
counted, not evidenced — see the note at the top).
...
The running census of all run-13+ survivors is kept as **counts per run
length** in the checkpoint (`near_counts`), printed in every `[STATUS]`
line and by `--status`; the per-value near-miss log the early legs wrote
was retired 2026-08-18. Census caveat: per-value counting was added
mid-run at p ≈ 8.7×10¹⁷. Below that point only the *first* prime of each
run length was recorded, so the run-13..16 counts undercount the range
below it; above it the run-13+ census is complete. A complete low-range
census can be regenerated by re-sweeping with the launcher if ever needed.
...
and 6 run-18 primes** (campaign totals 54 and 7), each verified three
ways when met, plus census counts runs 13/14/15/16 = 939/449/177/78. All
exact runs ≥ 13 are census-counted in the checkpoint.
...
primes** (campaign totals 270 and 34), each verified three ways when met,
and census counts above the u64 cap now run 13/14/15/16 =
6539/2738/1115/452.
...
run-17s, 14 run-18s (census counts) and the run-19 (`evidence/`).
Campaign totals at the
